# Plan Automation Directive

Clarify requirements and create a human-readable automation plan before converting to task-spec.json.

## Purpose

This is **Step 1** of the two-step automation workflow:
1. **Plan Automation** (this directive) → Creates readable plan, gets approval
2. **Create Automation** → Converts approved plan to task-spec.json
...
**Save to:** `automations/plans/{next-id}-{name}-plan.md`
...
- `automations/plans/{id}-{name}-plan.md` (approved plan)
- User approval to proceed

## Next Step

Once approved, run `directives/create-automation.md` to convert the plan to `task-spec.json`.
